/* rmnfb.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int rmnfb_(real *b, real *d__, real *fx, integer *iv, 
	integer *liv, integer *lv, integer *p, real *v, real *x)
{
    ...
    /* Local variables */
    static integer i__, j, k, w, g1, iv1, ipi;
    extern /* Subroutine */ int s3grd_(real *, real *, real *, real *, real *,
    ...
    extern /* Subroutine */ int rmngb_(real *, real *, real *, real *, 
	    integer *, integer *, integer *, integer *, real *, real *), 
	    ivset_(integer *, integer *, integer *, integer *, real *);
    static integer alpha0;
...
/*        THIS ROUTINE INTERACTS WITH SUBROUTINE   RMNGB  IN AN ATTEMPT */
...
/*        THE PARAMETERS FOR  RMNFB ARE THE SAME AS THOSE FOR   MNG */
...
/*     GRADIENT OF THE OBJECTIVE FUNCTION AT X,  RMNFB CALLS  S3GRD, */
...
/*  S3GRD... COMPUTES FINITE-DIFFERENCE GRADIENT APPROXIMATION. */
/*  RMNGB... REVERSE-COMMUNICATION ROUTINE THAT DOES   MNGB ALGORITHM. */
...
L20:
    rmngb_(&b[3], &d__[1], fx, &v[g1], &iv[1], liv, lv, p, &v[1], &x[1]);
    if ((i__1 = iv[1] - 2) < 0) {
	goto L999;
    } else if (i__1 == 0) {
	goto L30;
...
/*     ***  UNDO INCREMENT OF IV(NGCALL) DONE BY  RMNGB  *** */
    --iv[30];
/*     ***  STORE RETURN CODE FROM  S3GRD IN IV(SGIRC)  *** */
    iv[57] = 0;
...
L60:
    ...
    alpha = g1 - *p;
    w = alpha - 6;
    s3grd_(&v[alpha], &b[3], &d__[1], &v[42], fx, &v[g1], &iv[57], p, &v[w], &
	    x[1]);
    i__ = iv[57];
    if (i__ == 0) {
	goto L10;
