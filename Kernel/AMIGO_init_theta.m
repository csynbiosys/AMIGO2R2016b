...
if inputs.PEsol.n_global_theta>0
    ...
    if isempty(inputs.PEsol.global_theta_min) && isempty(inputs.PEsol.global_theta_max)
        if isempty(inputs.PEsol.global_theta_guess)==1
            inputs.PEsol.global_theta_guess=inputs.model.par(inputs.PEsol.index_global_theta); 
        end
    else
        if isempty(inputs.PEsol.global_theta_guess)==1
            inputs.PEsol.global_theta_guess=mean([inputs.PEsol.global_theta_max;inputs.PEsol.global_theta_min]);
        end
    end
