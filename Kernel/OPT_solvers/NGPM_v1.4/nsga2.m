...
function result = nsga2(opt, inputs,results,privstruct)
...
[pop, state] = evaluate(opt, pop, state, inputs,results,privstruct);
...
while( ngen < opt.maxGen && STOP_NSGA==0)
    ...
    [newpop, state] = evaluate(opt, newpop, state,inputs,results,privstruct);
