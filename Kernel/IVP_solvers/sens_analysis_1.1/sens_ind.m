% $Header: svn://.../trunk/AMIGO2R2016/Kernel/IVP_solvers/sens_analysis_1.1/sens_ind.m 770 2013-08-06 09:41:45Z attila $
function [tout,yout,tray_out,varargout] = sens_ind(odefile,tspan,y0,options,Uc,vecpar,initfile,varargin)
% 
% SENS_IND is an extension of the ODE15s (v 5.3) solver;as ODE15s, it
...
%
% [T,Y,DYDP] = SENS_IND('F',TSPAN,Y0,OPTIONS,P)
%
...
% 
% The VECPAR parameter tells SENS_IND whether the ode file has been "vectorized"
...
%
% [T,Y,DYDP] = SENS_IND('F',TSPAN,Y0,OPTIONS,P,VECPAR)
%
...
%
% [T,Y,DYDP] = SENS_IND('F',TSPAN,Y0,OPTIONS,P,VECPAR,initfile)
%
...
% may be passed
%
% [T,Y,DYDP] = SENS_IND('F',TSPAN,Y0,OPTIONS,P,VECPAR,initfile, varargin)
%
...
%   Mark W. Reichelt, Lawrence F. Shampine, and Jacek Kierzenka, 12-18-971%   Copyright (c) 1984-98 by The MathWorks, Inc.
...
% tolerances
% The 0.1 is added to avoid problems when Uc is close (or equal) to 0.
...
%% mio, para IND
%Us=Uc*ones(1,filU)
%Us=Us+diag(diag(Us))*sqrt(eps)
%% mio; escalar el sqrt(eps)
if n > 1
  ...
  if SE | ~issparse(Mt)
    ...
    for iiU=1:filU
     ...
     [y,yp,f0,dfdy,nFE,nPD,fac,g] = icdae(odefile,tspan,htry,Mt,y,f0,rtol,...
         Janalytic,jthresh,vectorized,Js,vararg{:});
    for iiU=1:filU
       [tray(:,iiU),trayp(:,iiU),ftray0(:,iiU),dfdy,nFE,nPD,fac,g] = icdae(odefile,tspan,htry,Mt,tray(:,iiU),ftray0(:,iiU),rtol,...
         Janalytic,jthresh,vectorized,Js,vararg{:});
     end  
    % cambiado varargin por vararg
  end
...
difdu(:,1,:)=h*yp*ones(1,filU);
%if vecpar
%   f0=feval(odefile,t,y,'',Uc)*ones(1,filU);
%   Ucn=Uc*ones(1,filU);
%   Ucn=Ucn+told*eye(filU);
%   tray=(feval(odefile,t,y,'',Ucn)-f0)/told;
%   difdu(:,:,1)=h*tray;
%else
% for iiU=1:filU
%   Ucn=Uc;Ucn(iiU)=Ucn(iiU)+told;
%   tray(:,iiU)=(feval(odefile,t,y,'',Ucn)-feval(odefile,t,y,'',Uc))/told;
%   difdu(:,iiU,1)=h*tray(:,iiU);
% end
%end
%difdu(:,:,1)=h*yp*ones(1,filU);
% VMGM / RGP fin
...
if nargout > 0
 ...
 %   VMGM / RGP i
   tray_out=zeros(ntspan,neq,filU);   
 % VMGM / RGP f
...
% VMGM / RGP f
 ...
 % VMGM / RGP i
      ...
      if neq>1
         predsen= tray+squeeze(sum(difdu(:,K,:),2));
      else
         predsen= tray+squeeze(sum(difdu(:,K,:),2))';         
      end
      traynew=predsen;
...
% VMGM / RGP f
      ...
      if Mtype == 2
        Mtnew = feval(odefile,tnew,ynew,'mass',vararg{:});
      end
   ...
   % VMGM / RGP i        
          if vecpar
             deld=hinvGak*feval(odefile,tnew,traynew,'',varargn{:})-Mtnew*(psisen+difkpsen);
             if DAE                          % Account for row scaling.
               deld = RowScale .* deld;
             end
          else          
           for iiU=1:filU
           varaux=[{Ucn(:,iiU)},varargin];
           deld(:,iiU)=hinvGak*feval(odefile,tnew,traynew(:,iiU),'',varaux{:})-Mtnew*(psisen(:,iiU)+difkpsen(:,iiU));
             if DAE                          % Account for row scaling.
...
% VMGM / RGP f
  ...
  if haveeventfun
    [te,ye,ie,valt,stop] = ...
        odezero('ntrp15s',odefile,valt,t,y,tnew,ynew,t0,vararg,h,dif,k);
    nte = length(te);
    if nte > 0
      if nargout > 2
        teout = [teout; te];
...
% VMGM / RGP i
        tray_out=[tray_out; zeros(chunk,neq,filU)];
% VMGM / RGP f
      end
      tout(nout) = tnew;
      yout(nout,:) = ynew.';
% VMGM / RGP i
      tray_out(nout,:,:)=traynew;
% VMGM / RGP f
...
% VMGM / RGP i
        for iiU=1:filU
           tray_out(nout,:,iiU)=ntrp15s(tspan(next), [],[], tstep, traystep(:,iiU),h,difdu(:,:,iiU),k);
        end   
% VMGM / RGP f
        next = next + 1;
      end
...
% VMGM / RGP i
for iiU=1:filU
   tray_out(:,:,iiU)=(tray_out(:,:,iiU)-yout)/told(iiU);
end
% VMGM / RGP f
