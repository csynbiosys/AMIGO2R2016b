function model = galCloseLoop1_model_George1()
...
cprintf('loading close loop model 1\n');
...
model.eqns=...                                      % Equations describing system dynamics. Time derivatives are regarded 'd'st_name''
               char('dL0=k_1*G11-2*k1*L0*G10+2*k_1*G12-k1*L0*G11+kLacI+k_1*G21-2*k1*L0*G20+2*k_1*G22-k1*L0*G21+k_2*L1-2*k2*L0*IPTGint-kd*L0',...
                    'dCitrine=kC*G20+lk*kC*(G21+G22)-kd*Citrine',...
                    'dLac12=lk*kC*(G11+G12)+kC*G10-kTP1*Lac12-kd*Lac12',...
                    ...
                    'dG10=k_1*G11-2*k1*L0*G10+kd*G11',...
                    'dG11=2*k1*L0*G10-k_1*G11+2*k_1*G12-k1*L0*G11+2*kd*G12-kd*G11',...
                    'dG12=k1*L0*G11-2*k_1*G12-2*kd*G12',...
