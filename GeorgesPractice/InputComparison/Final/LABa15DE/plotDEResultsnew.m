function minrec= plotDEResultsnew(resultList,max,F,CR,S,remap)
...
hold off;
if (remap)
    saveas(gcf,['remapped DE with F=',num2str(F),', CR=',num2str(CR),', S=',num2str(S),'.fig']);
else
    saveas(gcf,['raw DE with F=',num2str(F),', CR=',num2str(CR),', S=',num2str(S),'.fig']);
end
