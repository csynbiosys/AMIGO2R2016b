...
inputs.pathd.results_folder='Bioreactor';        % Folder to keep re(u1+u2)lts (in Re(u1+u2)lts) for a given problem
...
% my part
y0_init=inputs.DOsol.y0;
inputs.exps.n_exp = 1;

switching_time=switching_time_options(1);%(randi([1,4],1)); %randi([60*60,(t_d/3)],1);                        % 250 minutes in seconds


% Setting up input related parameters

inputs.exps.exp_type{1} = 'fixed';
inputs.exps.u_interp{1} = 'stepf';                               %OPTIONS:u_interp: 'sustained' |'step'|'linear'(default)|'pulse-up'|'pulse-down'
inputs.exps.t_f{1}      = 10.0;
inputs.exps.t_con{1}    = inputs.DOsol.t_con;                       % Input swithching times: Initial and final time
inputs.exps.t_s{1}      = inputs.DOsol.t_con; %(IPTGext_min:switching_time:exps.t_f{1});                                   % sampling time is 5 minutes (5X60=300 seconds)
inputs.exps.n_s{1}      = length(inputs.exps.t_s{1});                                % Number of sampling times
inputs.exps.exp_y0{1}   = y0_init;                                       %initial values of all states in the model


inputs.exps.n_steps{1}=20;
inputs.exps.u{1}(1,:)   = 0.5*ones(1,inputs.DOsol.n_steps);       % Initial guess for the input
inputs.exps.u_min{1}(1,:)= 0*ones(1,inputs.DOsol.n_steps);
inputs.exps.u_max{1}(1,:)= 1*ones(1,inputs.DOsol.n_steps);           % Minimum and maximum value for the input

inputs.exps.u{1}(2,:)=0.5*ones(1,inputs.DOsol.n_steps);        % Initial guess for the input
inputs.exps.u_min{1}(2,:)=0*ones(1,inputs.DOsol.n_steps);
inputs.exps.u_max{1}(2,:)=1.0*ones(1,inputs.DOsol.n_steps);           % Minimum and maximum value for the input


% number of observables
inputs.exps.n_obs{1}=1;
    
inputs.exps.u_delay_flag{1}=0;

% names of observables
inputs.exps.obs_names{1}=char('product');

% Observables definition
inputs.exps.obs{1}=char('product=y1*y4-0.0*y8');