...
for Time=1:Experiment_Length
    ...
    % Testing purpose
    Fluorescence_Record=yout(Time)-0.003;
    ...
    Denormalized_Error_Added_to_Input(Time)=mean(Denormalized_Error(Error_Reevaluaing_index+1:Time));
    %% PI controller
    if (rem(Time,200)==0)
        Cumulative_Error=Cumulative_Error+Ki*Denormalized_Error_Added_to_Input(Time);
    end
...
x=1:1000;
hold on;
plot(x,y0_math_model(1:1000));
%plot(x,r(1:1000));
plot(x,Denormalized_Input_to_IRMA(1:1000));
plot(x,yout(1:1000));
%plot(x,Normalized_Fluo_Out);    
plot(x,Actual_Input_Record(1:1000));
