...
Controller_Out_Record=zeros(1,Experiment_Length);
...
for Time=1:Experiment_Length
    ...
    Denormalized_Error(Time)=(r(Time)-Normalized_Fluo_Out(Time))*Normalize_Factor_IRMA;

    %% PI controller
    Integration=cumsum(Denormalized_Error);
    Controller_Out=Kp*Denormalized_Error(Time)+Ki*Integration(end);
    Controller_Out_Record(Time)=Controller_Out;
    ...
    if (    round((abs(yout_high-Controller_Out)),4) <=   round((abs(yout_low-Controller_Out)),4)   )
        u(Time)=1;
        y0=sol_high;
    else
        u(Time)=0;
