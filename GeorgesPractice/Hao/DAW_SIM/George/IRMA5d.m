...
function dy = IRMA5d(t,y,Z,K,u,period,tmin)
...
if ((((t-tmin)/period)+1)>=length(u))
    ut=(u(end)==1);
else
    ut=(rem(t-tmin,1)<=u(floor((t-tmin)/period)+1));
end
...
% or glucose)
dy(3,1) =(K(11)*alfa3+ (K(12)*(1-ut)+ut*(K(12)*K(25)) )  *(y(2).^4./(  (K(14)*(1-ut)+ut*(K(14)/K(27)) ).^4 +y(2).^4  .*( 1+(y(4).^4./(   (K(13)*(1-ut)+ut*(K(13)*K(26)) ).^4)  ))  )    ) -deg3*K(15)*y(3));
% GAL80 mRNA
