...
for iexp=1%1:length(exps_indexall)
    exp_indexData = exps_indexall(7);
...
%% Compile and run the model with the Inducible promoter estimates on Stelling data
...
for iexp=1%1:length(exps_indexall)
    ...
    if exp_indexData<13
       exps.u{iexp} =  [0 Data.input{1,exp_indexData}];
       exps.exp_y0{iexp} =  InduciblePromoter_steady_state_ForStellingData(inputs.model.par,0);

    else
       exps.u{iexp} = [1000 Data.input{1,exp_indexData}];
       exps.exp_y0{iexp} =  InduciblePromoter_steady_state_ForStellingData(inputs.model.par,1000);
...
AMIGO_Prep(inputs);
sim_results_IP = AMIGO_SObs(inputs);
...
for e= 1:exps.n_exp
    ...
    plot(sim_results_Cross.sim.tsim{1,e}/3600,sim_results_Cross.sim.obs{1,e},'r'); hold on 
    plot(sim_results_IP.sim.tsim{1,e}/3600,sim_results_IP.sim.obs{1,e},'g');
    xlabel('Time (hours)');
    ylabel('Citrine (AU)');
    title(int2str(e))
end
