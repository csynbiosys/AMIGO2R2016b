...
for i=1:length(Data.expName)
    ...
    if contains(Data.expName{1,i},'up')
        ...
        Data.t_samples{1,i} = [0 12 24 36 48].*3600; % sampling times in seconds
        Data.t_con{1,i} = [0 48].*3600; % tcon in seconds
        ...
        Data.standard_dev{1,i} = Data.iqr{1,i}/(2*0.6746); 
    else
        Data.input{1,i} = str2double(Name{1,2}); 
        Data.n_samples{1,i} = 5; 
