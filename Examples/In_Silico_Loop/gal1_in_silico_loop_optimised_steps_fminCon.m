...
for i=1:numLoops
   ...
   % final time constraint 
    ...
    inputs.nlpsol.eSS.local.solver =0;% 'fminsearch'; % note that, in order to handle constraints, an SQP approach is required (e.g. fminsearch cannot be used). 
    ...
    inputs.nlpsol.eSS.log_var=[1:1:inputs.exps.n_steps{iexp}];
    inputs.plotd.plotlevel='full';
