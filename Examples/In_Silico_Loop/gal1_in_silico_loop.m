...
% Loop for 1 times 60 hour experiment
for i=1:1
    ...
    inputs.exps.t_f{iexp}=3600;                              % Duration 60 hours
    inputs.exps.n_s{iexp}=3600/5+1;                          % Number of sampling times - sample every 5 min

    % OED of the input 
    inputs.exps.u_type{iexp}='od';
    inputs.exps.u_interp{iexp}='stepf';                       % Stimuli definition for experiment
    inputs.exps.n_steps{iexp}=100;                            % Number of steps
    ...
    inputs.nlpsol.eSS.maxeval = 2000000;
