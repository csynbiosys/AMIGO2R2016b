---
title: "Comparison of 10 6-hour experiments and one 60-hour experiment."
author: "Ally Hume"
...
Here we compare two experiment to show the power of closed loop optimisation.  One experiment (Greedo - labelled 'M' for multiple) plans 10 6-hour experiments using OID, Experiment, PE and updates knowledge of the previous experiments and the current best parameter estimate to choose the next experiment.  The other experiment (Chewbacca - labelled 'S' for single) simply plans a 60-hour experiment in advance.
...
load_experiment_data <- function(filename) {
  ...
  exptName <- filename
  exptName <- gsub('Greedo',    'M',  exptName)
  exptName <- gsub('Chewbacca', 'S',  exptName)
...
```{r echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
# Get the data files.
files <- c(list.files(pattern='Greedo.*dat'),list.files(pattern='Chewbacca.*dat'))
...
```{r echo=FALSE}
d <- data %>% filter(type == 'PARAM_FIT' & param == 'alpha1' &  ((iteration == 10 & exptName == 'M') |  (iteration == 1 & exptName == 'S')))
...
```{r echo=FALSE}
d <- data %>% filter(type == 'PARAM_FIT' & ((iteration == 10 & exptName == 'M') |  (iteration == 1 & exptName == 'S')) ) %>% 
...
```{r echo=FALSE}
d <- data %>% filter(type == 'REL_CONF' & ((iteration == 10 & exptName == 'M') |  (iteration == 1 & exptName == 'S')) ) %>% 
     select(exptName, param, trueValue, value) 
