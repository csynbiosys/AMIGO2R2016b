% $Header: svn://.../trunk/AMIGO2R2016/Postprocessor/AMIGO_del_PE.m 2482 2016-02-11 14:36:32Z evabalsa $
% AMIGO_del_PE: Clear all unnecessary variables in the workspace
%
%******************************************************************************
...
inputs=rmfield(inputs,char('rid','rank'));
if(isfield(results,'rank'))
    results=rmfield(results,char('rank'));
end

if exist('fobj_fsqp.m' )==2
    delete fobj_fsqp.m; end
if exist('fobj_dn2fb.m' )==2
    delete fobj_dn2fb.m; end
if exist('fobj_n2fb.m' )==2
    delete fobj_n2fb.m; end
if exist('objf_fsqp.m' )==2
    delete objf_fsqp.m; end
...
% Do not delete objective functions since in case of Jpar, an other thread may use it.:
% if exist('objf_nl2sol.m' )==2
%     delete objf_nl2sol.m; end
% if exist('fjac_nl2sol.m' )==2
%     delete fjac_nl2sol.m; end